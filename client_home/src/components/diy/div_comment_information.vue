<template>
	<view class="diy_details diy_div_comment_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','comment_time')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>评论时间:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["comment_time"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','rating_value')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>评分值:</span>
						</view>
					...
					<view v-if="$check_field('get','comment_details')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>评论详情:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/comment_information/get_obj", {
					comment_information_id:this.query.comment_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["comment_information_id"] !==0) {
...
<style scoped>
	.div_comment_information_details {}
