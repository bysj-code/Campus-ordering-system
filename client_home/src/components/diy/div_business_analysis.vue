<template>
	<view class="diy_details diy_div_business_analysis">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','total_revenue')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>收入总额:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/business_analysis/get_obj", {
					business_analysis_id:this.query.business_analysis_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["business_analysis_id"] !==0) {
