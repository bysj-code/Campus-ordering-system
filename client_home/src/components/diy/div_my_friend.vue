<template>
	<view class="diy_details diy_div_my_friend">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','friend_no')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>好友编号:</span>
						</view>
							<view class="diy_field diy_uid">
							<text>
								{{ get_user_friend_no(obj['friend_no']) }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','friend_name')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>好友名称:</span>
						</view>
					...
					<view v-if="$check_field('get','friend_type')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>好友类型:</span>
						</view>
							<view class="diy_field diy_text">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/my_friend/get_obj", {
					my_friend_id:this.query.my_friend_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["my_friend_id"] !==0) {
