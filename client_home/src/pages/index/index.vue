<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="餐品推荐推荐"
      url="/pages/food_recommendation/list"
      :list="list_article"
      v-if="$check_action('/food_recommendation/list', 'get')"
    >
	  <list_food_recommendation :list="list_food_recommendation"></list_food_recommendation>
...
<script>
								...
								import list_food_recommendation from "@/components/diy/list_food_recommendation.vue";
      ...
      this.isAdmin = !this.isAdmin;
		...
		*/
		get_food_recommendation() {
			let url = "~/api/food_recommendation/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("餐品推荐" ,json.result.list);
					this.list_food_recommendation = json.result.list;
											this.list_food_recommendation.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_food_recommendation ,"food_recommendation" ,"34685");
									}
			});
		},
