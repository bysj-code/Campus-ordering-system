<template>
	<view class="page_food_recommendation diy_detail diy_detail--new" id="food_recommendation_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','seller_no')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>商家编号</span>
							</view>
					...
					<view v-if="$check_field('get','store_name')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>店铺名称</span>
							</view>
					...
					<view v-if="$check_field('get','detailed_introduction')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								...
								<rich-text class="rich_text" :nodes="obj['detailed_introduction']"></rich-text>
							</view>
							</view>
					</view>
				</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=food_recommendation&source_field=food_recommendation_id&source_id=' + obj['food_recommendation_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "food_recommendation",
					source_field: "food_recommendation_id",
					source_id: obj["food_recommendation_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "food_recommendation",
					source_field: "food_recommendation_id",
					source_id: this.obj["food_recommendation_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/food_recommendation/set?food_recommendation_id=' + _this.obj["food_recommendation_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/food_recommendation/set?food_recommendation_id=' + obj["food_recommendation_id"], {
					hits
				}, (res) => {
					if(res.result){
