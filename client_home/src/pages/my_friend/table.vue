<template>
  <view class="page_my_friend diy_table diy_table_tml" id="my_friend_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="好友编号" name="friend_no">
          <uni-data-select
          placeholder="请选择"
          v-model="query['friend_no']"
          :localdata="list_friend_no"
      ></uni-data-select>
                    </uni-forms-item>
                      </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/my_friend/view?" v-show="user_group == '管理员' || $check_action('/my_friend/table','add') || $check_action('/my_friend/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','friend_no')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>好友编号</span>
							</view>
															<view class="diy_field diy_uid text">
								<span>
									{{ get_user_friend_no(o['friend_no']) }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="1 && $check_field('get','friend_name')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>好友名称</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','friend_type')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>好友类型</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/my_friend/table','del') || $check_action('/my_friend/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/my_friend/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/my_friend/table','set') || $check_action('/my_friend/view','set') || $check_action('/my_friend/view','get')" class="btn btn-primary">详情</navigator>
					                    									</view>
			</view>
		</view>
	</view>
    ...
    return param;
        ...
        async get_list_user_user_number() {
          ...
          async get_list_friend_no() {
            var json = await this.$get("~/api/user/get_list?user_group=注册用户");
            if(json.result && json.result.list){
              json.result.list.map((o) => this.list_friend_no.push({value:o.user_id,text:o.nickname + '-' + o.username}));
            }
            else if(json.error){
              console.error(json.error);
            }
