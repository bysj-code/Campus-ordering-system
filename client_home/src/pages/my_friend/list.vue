<template>
	<view class="page_my_friend diy_list diy_list_page div_list_tml" id="my_friend_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
											<uni-section title="好友编号" type="line">
							<uni-data-select
									@change="change_friend_no"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?my_friend_id=' + o['my_friend_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
			...
			return {
				url_get_list: "~/api/my_friend/get_list?like=0",
					...
					list_user_user_number: [],
										// 用户列表
					list_user_friend_no: [],
							list_friend_no: [{value:"全部",text:"全部"}],
	...
	get_user_user_number(id){
		...
		return ret;
							...
							// 改变分类标签
	change_friend_no(val) {
		if (val === "全部" || val === "") {
			this.query.friend_no = "";
		}else{
			this.query.friend_no = val
		}
		this.search();
	},
